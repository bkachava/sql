...
SELECT country_id, c.country FROM country AS c WHERE (c.country) IN ('Afghanistan', 'Bangladesh', 'China');
...
# Verify that the new column was deleted
SELECT * FROM actor;
...
SELECT last_name, COUNT(*) AS total FROM actor GROUP BY last_name HAVING total > 1;

# 4c. The actor `HARPO WILLIAMS` was accidentally entered in the `actor` table as `GROUCHO WILLIAMS`. Write a query to fix the record.
# Verify how many record with this information exists
...
SELECT title, COUNT(actor_id) AS Number 
FROM film
INNER JOIN film_actor ON film.film_id = film_actor.film_id
GROUP BY actor_id;

# 6d. How many copies of the film `Hunchback Impossible` exist in the inventory system?
SELECT COUNT(film.film_id) AS Copies
FROM inventory, film
WHERE inventory.film_id = film.film_id AND title ='Hunchback Impossible';
# To verify the results of the previous statement
